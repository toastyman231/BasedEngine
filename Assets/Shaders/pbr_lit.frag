...
void main() {
    ...
    for (int i = 0; i < NR_POINT_LIGHTS; ++i) {
        ...
        if (renderMode == RenderMode_DiffuseDebug) {
            Lo += reflection.diffuse * NdotL;
        } else if (renderMode == RenderMode_SpecularDebug) {
            Lo += reflection.specular * NdotL;
        } else if (renderMode == RenderMode_ClearCoatDebug) {
            Lo += reflection.clearcoat * NdotL;
        } else {
            Lo += (lightColor * (reflection.diffuse + reflection.specular + reflection.clearcoat)) * NdotL
                /*** (1.0 - shadow)*/ * light.intensity;
        }
    }
    ...
    if (directionalLight.color != vec3(0.0)) {
        ...
        if (renderMode == RenderMode_DiffuseDebug) {
            Do = res.diffuse * NdotL;
        } else if (renderMode == RenderMode_SpecularDebug) {
            Do = res.specular * NdotL;
        } else if (renderMode == RenderMode_ClearCoatDebug) {
            Do = res.clearcoat * NdotL;
        } else {
    ...
    if (renderMode == RenderMode_Lit) {
        ...
        outColor = vec4(vec3(Metallic * GetMaterialMetallic(uvs)), 1.0);
    } else if (renderMode == RenderMode_RoughnessDebug) {
        outColor = vec4(vec3(Roughness * GetMaterialRoughness(uvs)), 1.0);
    ...
    } else if (renderMode == RenderMode_DiffuseDebug || renderMode == RenderMode_SpecularDebug
