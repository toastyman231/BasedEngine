...
{
    // Test mesh
    static std::vector<float> Vertex1{ 0.5,  0.5f, 0.f };
    static std::vector<float> Vertex2{ 0.5, -0.5f, 0.f };
    static std::vector<float> Vertex3{ -0.5, -0.5f, 0.f };
    static std::vector<float> Vertex4{ -0.5,  0.5f, 0.f };
    ...
    std::shared_ptr<Gameobject> CreatePaddle(const glm::vec2& pos)
    {
        ...
        vb->SetLayout({ 3 });
        std::shared_ptr<based::graphics::VertexArray> va = std::make_shared<based::graphics::VertexArray>();
        va->PushBuffer(std::move(vb));
        va->SetElements(Elements);
        va->Upload();
        std::shared_ptr<based::graphics::Shader> shader = std::make_shared<based::graphics::Shader>(VertexShader, FragmentShader);
    ...
    std::shared_ptr<Ball> CreateBall(const glm::vec2& pos)
    {
        ...
        va->SetElements(Elements);
        va->Upload();
        std::shared_ptr<based::graphics::Shader> shader = std::make_shared<based::graphics::Shader>(VertexShader, BallFragmentShader);
    ...
    std::shared_ptr<Gameobject> CreateBG()
    {
        ...
        va->SetElements(Elements);
        va->Upload();
        std::shared_ptr<based::graphics::Shader> shader = std::make_shared<based::graphics::Shader>(VertexShader, BGFragmentShader);
