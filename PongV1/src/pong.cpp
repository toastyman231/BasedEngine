...
public:
	...
	void Initialize() override
	{
		...
		mLeftPaddleUp = BASED_INPUT_KEY_W;
		mLeftPaddleDown = BASED_INPUT_KEY_S;
		mRightPaddleUp = BASED_INPUT_KEY_UP;
		mRightPaddleDown = BASED_INPUT_KEY_DOWN;
	...
	void Update() override
	{
		...
		if (mInGamplay)
		{
			...
			if (mBall->GetPos().y >= mBallMaxY || mBall->GetPos().y <= mBallMinY)
			{
				mBall->FlipVelY();
			}

			if (IsColliding(mBall->GetPos(), mBall->GetSize(), mLeftPaddle->GetPos(), mLeftPaddle->GetSize())
				&& !mBallCollisionLeft)
			{
				mBall->FlipVelX();
			...
			if (IsColliding(mBall->GetPos(), mBall->GetSize(), mRightPaddle->GetPos(), mRightPaddle->GetSize())
				&& mBallCollisionLeft)
			{
				mBall->FlipVelX();
				mBallCollisionLeft = !mBallCollisionLeft;
