...
def make_output_path(input_path, project_root, destination_root, extension_override=""):
    ...
    # Binaries in PostBuildCopy/PLATFORM/ are stripped of both components so
    # they land directly in the output root alongside the exe, e.g.:
    #   engine/PostBuildCopy/Windows/foo.dll  ->  bin/Release/foo.dll
    parts = list(output_path.parts)
    for i, part in enumerate(parts):
        if part == "PostBuildCopy" and i + 1 < len(parts):
            # Drop "PostBuildCopy" and the platform subdirectory that follows it
            filtered_parts = parts[:i] + parts[i + 2:]
